...
output_dir = os.path.join(r'C:\Users\boldr\Downloads\chesspdftofen-master', base_name)
...
def process_page(image, page_idx):
    print(f"Processing page {page_idx + 1}")  # Print the current page number
    ...
    regions = [
        ...
        (0, height // 2, 0, width // 2),  # Top-left diagonal strip
        (0, height // 3, 0, width // 3),  # Top-left quarter
        (0, height // 2, width // 2, width),  # Top-right diagonal strip
        (0, height, width // 2, width),  # Right half
        (height // 2, height, 0, width),  # Bottom half
        # (height // 3, 2 * height // 3, width // 3, 2 * width // 3),  # Center rectangle (removed)
        ...
        (height // 2, height, 0, width // 2),  # Bottom-left diagonal strip
        (height // 2, height, width // 2, width),  # Bottom-right diagonal strip
        (height // 4, 3 * height // 4, width // 3, 2 * width // 3)  # Central vertical band
    ]

    # Track seen board coordinates with page index
    seen_boards = set()
    ...
    for region_idx, (y1, y2, x1, x2) in enumerate(regions):
        ...
        # Save the processed image of the region
        ...
        # Detect boards in the processed image
        ...
        for board_idx, board in enumerate(boards):
            ...
            # Create a unique identifier for the board including the page index
            board_id = (page_idx, adjusted_board_y1, adjusted_board_y2, adjusted_board_x1, adjusted_board_x2)
            ...
            if board_id not in seen_boards:
                ...
                print(f"Duplicate board detected and skipped in page {page_idx + 1}, region {region_idx + 1} board {board_idx + 1}")

    if not seen_boards:
        print(f"No boards detected in page {page_idx + 1}")
